...
SELECT
  l.*,
  -- Select all columns from the penguins table (l)
...
LEFT JOIN
  island_details r
ON
  l.island = r.island;  -- Left join to keep all penguins, even if no country match
...
JOIN
  island_details r
ON
  l.island = r.island  -- Inner join, only keep matching islands
GROUP BY
  1,
  2
ORDER BY
...
WITH tags AS (
  ...
  UNION ALL
  SELECT
    'Torgersen' AS island, ['gray', 'nostalgic'] AS tags 
)
SELECT
  l.*,
  r.tags
...
JOIN tags r ON l.island = r.island
JOIN
  UNNEST(tags) AS tag  -- Flatten the tags array
LIMIT
  10;